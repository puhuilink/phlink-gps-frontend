        ...
        >
            ...
            <el-table-column label="模型类别" prop="category" align="center">
                <template slot-scope="scope">
                    <span v-html="myFormatDictText(dicts.processCategory,scope.row.category)"></span>
                </template>
            </el-table-column>
            ...
            <el-form ref="dataForm" :rules="rules" :model="temp" :disabled="dialogStatus==='view'"
                ...
                <el-form-item label="模型类别" prop="category">
                    <el-select v-model="temp.category" placeholder="模型类别">
                        <el-option v-for="item in dicts.processCategory" :key="item.itemValue" :label="item.itemText"
...
<script>
    import {importModel, deployModel, getModelList, saveModel, updateModel, deleteModel} from '@/api/flowable-model'
    import {queryDictItemByDictCode} from '@/api/dict'
    import {formatDictText} from '@/utils/string'
    ...
    export default {
        ...
        beforeCreate() {
            // todo
            queryDictItemByDictCode('processCategory').then(({data}) => {
                this.dicts['processCategory'] = data.data
            })
            queryDictItemByDictCode('taskCategory').then(({data}) => {
                this.dicts['taskCategory'] = data.data
            })
        },
        created() {
            this.list()
        ...
        methods: {
            ...
            btnUpdateModel(row) {
                this.$router.push({path: '/popup/modelEdit', query: {id: row.id}})
            },
            updateData() {
                this.$refs['dataForm'].validate((valid) => {
                    if (valid) {
                        updateModel(this.temp).then(({msg}) => {
                            this.dialogFormVisible = false
                            this.$message({
                                type: 'success',
                                message: msg
            ...
            btnDelete(id, cascade) {
                ...
                deleteModel({ids: ids.toString(), cascade}).then(({msg}) => {
                    this.$message({
                        type: 'success',
                        message: msg
                    })
            ...
            myFormatDictText(data) {
                return formatDictText(data)
            }
        }
    }
</script>