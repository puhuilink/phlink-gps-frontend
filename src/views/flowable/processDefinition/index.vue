...
<script>
    import {list, importDef, updateDef, deleteDef} from '@/api/flowable-definition'
    ...
    export default {
        ...
        components: {ProcessImage, ProcessDefinitionIdentityLink},
        data() {
            return {
                dicts: [],
                records: null,
        ...
        computed: {
            getSuspended: function() {
                return this.temp.suspended?'挂起':'激活'
            }
        },
        methods: {
            list() {
            ...
            btnSuspendOrActivate(processDefinitionId, suspend) {
                ...
                updateDef(suspendOrActivate, {processDefinitionId}).then(({msg}) => {
                    this.$message({
                        type: 'success',
                        message: msg
                    })
