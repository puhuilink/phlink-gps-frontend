...
<script>
    ...
    import {getList, queryById, claim, unclaim} from '@/api/flowable-task'

    export default {
        name: 'FlowableTask',
        components: {TaskDetail, ExecuteTask, ProcessDetail},
        data() {
            return {
                dicts: [],
                records: null,
        ...
        methods: {
            ...
            btnViewTask(row) {
                queryById({taskId: row.id}).then(res => {
                    const {data} = res.data
            ...
            btnUnclaim(row) {
                unclaim({taskId: row.id}).then(({msg}) => {
                    this.$message({
                        type: 'success',
                        message: msg
                    })
