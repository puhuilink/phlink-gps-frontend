                ...
                <el-form-item label="抄送给:">
                    <el-row align="left">
                        ...
                        <el-col :span="23" align="left">
                            <el-tag v-for="tag in ccToVos" :key="tag.username" effect="plain" closable
...
<script>
    ...
    import {start} from '@/api/flowable-process'
    import {renderedStartForm} from '@/api/flowable-definition'
        ...
        methods: {
            initData() {
                if (this.processDefinition.formKey) {
                    renderedStartForm({processDefinitionId: this.processDefinition.id}).then(res => {
                        const {data} = res.data
