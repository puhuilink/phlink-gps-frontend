...
<script>
    ...
    import {formData, comments} from '@/api/flowable-process'
        ...
        methods: {
            ...
            showProcessInstanceForm() {
                if (!this.isShowProcessInstanceForm) {
                    formData({processInstanceId: this.processInstanceId}).then(res => {
                        const {data} = res.data
            ...
            listComments() {
                comments({processInstanceId: this.processInstanceId}).then(res => {
                    const {data} = res.data
                    this.comments = data;
                })
            },
            showImage() {
                if (!this.imagePath) {
                    this.imagePath = `${process.env.BASE_API}` + '/flowable/processInstanceImage?processInstanceId=' + this.processInstanceId + '&access_token=' + getToken() + '&time=' + new Date()
                }
            }
        }
    }
