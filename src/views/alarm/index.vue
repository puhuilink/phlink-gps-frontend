    ...
    >
      ...
      >
        ...
        <el-form-item label="围栏ID" :label-width="formLabelWidth" prop="fenceId">
          <input v-model="dataForm.fenceId" type="hidden">
          <el-select
            v-model="dataForm.fenceInfo.name"
            v-loadmore="loadMoreFence"
            filterable
            placeholder="请输入围栏名称"
            remote
            clearable
...
import { getFence } from '@/api/fence'
...
export default {
  ...
  methods: {
    ...
    // TODO
    changeNameToId(data) {
      this.dataForm.fenceId = data
    },
    loadMoreFence: function() {
      const params = new URLSearchParams()
      this.iotFencePageParam.current = this.iotFencePageParam.current + 1
      if (this.dataForm.fenceInfo.name !== '') {
        params.append('name', this.dataForm.fenceInfo.name)
      }
      params.append('current', this.iotFencePageParam.current)
      getFence(params).then(res => {
        this.fenceOptions = res.data.data.records.map(obj => {
          return {
            id: obj.id,
            fence: obj.fence,
            name: obj.name
