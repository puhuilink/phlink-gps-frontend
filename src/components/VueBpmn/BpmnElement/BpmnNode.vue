                ...
                <el-collapse-item name="1">
                    ...
                    <el-form-item v-if="!!showConfig.taskCategory" label="任务分类" prop="taskCategory">
                        <el-select v-model="formData.category">
                            <el-option v-for="item in this.taskCategory" :label="item.itemText" :key="item.itemValue"
        ...
        created() {
            ...
            if(this.element.businessObject.extensionElements) {
                this.executionListenerLength = this.element.businessObject.extensionElements.values
                    .filter(item => item.$type === (this.descriptorPrefix + 'ExecutionListener')).length
                this.taskListenerLength = this.element.businessObject.extensionElements.values
                    .filter(item => item.$type === (this.descriptorPrefix + 'TaskListener')).length
            }
        },
        methods: {
